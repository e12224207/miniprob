...
import { BinaryExpression, LogicalNegation, IntLiteral, BoolLiteral, ProbabilisticAssignment, Lval, Assignment, Distribution, isIntLiteral, isBoolLiteral, isLval, isLogicalNegation, isBinaryExpression, isDistribution, isProbabilisticAssignment, isAssignment, DeclOrParam, isType, Type, isParam, isIntType, isIntArray } from "../generated/ast.js";
import { IntegerTypeDescription, TypeDescription, createArrayType, createBooleanType, createDistributionType, createErrorType, createIntegerType, isErrorType, isIntegerType, typeToString } from "./description.js";
    ...
    if (isBoolLiteral(node)) {
        ...
        try {
            ...
            if ((interpetedSuffix.width >> 29) !== 0) {
                ...
                type = createErrorType(`Value ${node.literal.value} does not fit into integer with width ${interpetedSuffix.width}`, node);
            } else {
...
function inferDistribution(distribution: Distribution, cache: Map<AstNode, TypeDescription>): TypeDescription {
    ...
    switch (distribution.name) {
        ...
        case "Uniform":
            const upperType = inferType(distribution.upper, cache);
...
function inferProbabilisticAssignment(assignment: ProbabilisticAssignment, cache: Map<AstNode, TypeDescription>): TypeDescription {
    ...
    if (!fallbacks.every(typeDesc => typeDesc.$type === head.$type)) {
        return createErrorType(`Possible results do not fit expected type of \'${typeToString(head)}\'.`, assignment);
    }

    if (isIntegerType(head) && fallbacks.every(val => isIntegerType(val))) {
        fallbacks.push(head);
        const castFallbacks = fallbacks as IntegerTypeDescription[];
        const maxWidth = castFallbacks
            .reduce((best, current) => (current.signed ? current.width - 1 : current.width) > (best.signed ? best.width - 1 : best.width) ?
                current : best);
        return maxWidth;
    }
...
function inferFromType(
    ...
    if (isIntType(type)) {
        ...
        if (isIntArray(type)) {
            if (!indexedAccess)
                return createArrayType(createIntegerType(width, signed));
        }
